...
func master(c *cli.Context) {
	...
	engine := gin.Default()
	e := engine.Group("/")
	e.POST("/apps/:app_key/events", apiEvents)
	e.POST("/apps/:app_key/batch_events", apiBatchEvents)
	e.GET("/apps/:app_key/channels", apiChannels)
	e.GET("/apps/:app_key/channels/:channel", apiChannel)
	e.GET("/apps/:app_key/channels/:channel/users", channelUsers)
